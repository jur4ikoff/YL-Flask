...
def get_one_jobs(jobs_id):
    ...
    return jsonify(
        {
            'jobs': jobs.to_dict(only=('team_leader', 'job', 'work_size', 'collaborators', 'is_finished'))
        }
    )
...
def create_jobs():
    db_session.global_init("db/register.db")
    db_sess = db_session.create_session()
    jobs_id = db_sess.query(Jobs).all()
    if not request.json:
        return jsonify({'error': 'Empty request'})
    elif not all(key in request.json for key in
                 ['id', 'team_leader', 'job', 'work_size', 'collaborators', 'is_finished']):
        return jsonify({'error': 'Bad request'})
    elif request.json["is_finished"] not in (0, 1):
        return jsonify({'error': 'is_finished not Boolean'})
    elif request.json:
